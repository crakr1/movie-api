...
exports.addReview = async (req, res) => {
    ...
    if (isRated > -1) 
    return res.status(403).send({message: 'yuo ar review this movie befor' })

    const totalRete = movie.reviews.reduce((sum, review) => sum = review.rate, 0)
    const finalRate = (totalRete + rate) / (movie.reviews.length + 1)
